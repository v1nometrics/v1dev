...
Os textos viram embeddings de 768 dimensões; a dimensionalidade é reduzida antes da clusterização para tornar a estrutura semântica mais tratável. Em seguida, um método baseado em densidade agrupa os pontos, com parâmetros ajustados para cada conjunto de dados.

<Flow
  caption="do texto ao rótulo: a llm só entra no fim"
  steps={[
    { label: "embeddings (768d)", note: "representação semântica do comentário (+ contexto quando necessário)" },
...
Não se trata de gerar embeddings e rodar um algoritmo de clusterização. Cada etapa carrega uma decisão: como representar o texto, quando fundir grupos quase iguais, quando a amostra é pequena demais para forçar estrutura, e se a coesão entre clusters passa de "matematicamente ok" para "útil no painel". Só depois disso a LLM entra, e só para rotular o que já foi formado.
...
Os principais assuntos de uma semana não são necessariamente os mesmos de um mês. A conversa de uma plataforma pode ser completamente diferente daquela encontrada em outra. Um agrupamento global e permanente perderia justamente a variação temporal que torna os tópicos relevantes.
