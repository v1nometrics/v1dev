...
Os resultados não são aceitos apenas porque o algoritmo conseguiu separar alguns grupos: a qualidade da estrutura é avaliada por métricas de coesão interna e separação entre clusters, e grupos semanticamente próximos são comparados e, quando necessário, fundidos — para que o produto não mostre três tópicos que são, na prática, a mesma conversa com sotaques diferentes.
...
Não se trata, portanto, de gerar embeddings e rodar um algoritmo de clusterização. O resultado depende de uma sequência de decisões sobre representação, dimensionalidade, densidade, validação, redundância, contexto e interpretação — construídas, testadas e corrigidas até que os tópicos deixassem de ser apenas matematicamente plausíveis e passassem a ser realmente úteis.
...
Também precisa conviver com filas persistentes, concorrência, idempotência, recuperação de execuções interrompidas, limites de custo, circuit breaker, canário, versionamento de imagem e rollback.
