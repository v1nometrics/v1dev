...
<Delta
  ...
  to="~64"
  unit="s"
  caption="o modelo não piorou; a contenção sob carga concorrente, sim"
/>
...
Foi aí que minha visão sobre o valor do pipeline mudou. O valor está menos no algoritmo de clusterização, na redução de dimensionalidade, nas heurísticas, nos pesos ou na quantidade de métricas do que no contrato construído ao redor do problema. Todas aquelas peças são substituíveis.
...
Isso não obriga a preservar o pipeline atual para sempre; só exige que qualquer substituto compita com o sistema inteiro, não só com o resultado mais visível dele.
...
Depois dessa reflexão, minha conclusão não foi escolher entre ciência de dados tradicional e LLMs, e sim parar de tratá-las como abordagens concorrentes.
