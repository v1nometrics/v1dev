---
title: "Vetores em produção: por que SBERT + Postgres (pgvector) ainda é uma escolha sólida"
date: "2026-01-15"
tags: ["ml", "nlp", "search", "postgres"]
summary: "Uma visão pragmática de vetores, busca vetorial e trade-offs de operação com Postgres + HNSW."
draft: true
---

## TL;DR
- Vetores são uma interface matemática estável para busca e matching semântico.
...
## Por que vetorização (e não LLM generativo)?

Vetores são **discriminativos**: você compara posições no espaço e toma decisões reproduzíveis.

LLMs são ótimos para geração, mas trazem variância e custo para tarefas que às vezes são puramente de similaridade.

<Callout type="note">
A vetorização transforma texto em vetores de dimensão fixa. A "semântica" fica codificada na posição do vetor no espaço.
</Callout>
